...
&emsp;&emsp;(2)更换被解释变量。一是考虑到基础结论更多说明的是数字技术创新的短期市场价值，`保留主并方每年首次并购完成的样本，将被解释变量分别替换为企业并购完成后第二年相较于并购发起前一年$`ROA`$的变化△ROA和$`EPS`$的变化△EPS,以证实数字技术创新的价值不仅存在于短期`；二是考虑到当前结果可能是由于投资者热点追捧或概念炒作行为所导致的，利用行业收益率调整后的超额收益重新计算$`CAR`$,回归结果依然稳健。<br>
```stata
**** 附表8 第1-5列
